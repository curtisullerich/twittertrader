 ...
 * user-provided domain knowledge (i.e., "feature labels").
 * 
 * Prior knowledge is incorporated via simple pseudocounts on the feature
 * multinomials.
 * 
 * This class implements a MALLET trainer class for NaiveBayes classifiers,
 * modified from the trainer in the MALLET source distribution.
 * 
 * @see NaiveBayes, NaiveBayesTrainer
 * @author Burr Settles
 */
public class NaiveBayesWithPriorsTrainer extends ClassifierTrainer<NaiveBayes>
		implements ClassifierTrainer.ByInstanceIncrements<NaiveBayes>,
		Boostable, AlphabetCarrying, Serializable {
	...
	double docLengthNormalization = -1; // A value of -1 means don't do any
										// document length normalization
	NaiveBayes classifier;

	// If this style of incremental training is successful, the following
	// members
	// should probably be moved up into IncrementalClassifierTrainer
	Pipe instancePipe; // Needed to construct a new classifier
	Alphabet dataAlphabet; // Extracted from InstanceList. Must be the same for
							// all calls to incrementalTrain()
	Alphabet targetAlphabet; // Extracted from InstanceList. Must be the same
								// for all calls to incrementalTrain

	// These are used to compute background knowledge on features
	Multimap<Object, Object> labelFeatures; // data structure storing a map from
											// labels to features
	...
	/**
	 * Get the MultinomialEstimator instance used to specify the type of
	 * estimator for features.
	 * 
	 * @return estimator to be cloned on next call to train() or first call to
	 *         incrementalTrain()
	 */
	 ...
	 * is internally cloned and the clone is used to maintain the counts that
	 * will be used to generate probability estimates the next time train() or
	 * an initial incrementalTrain() is run. Defaults to a
	 * Multinomial.LaplaceEstimator()
	 * 
	 * @param me
	 *            to be cloned on next call to train() or first call to
	 *            incrementalTrain()
	 */
	public NaiveBayesWithPriorsTrainer setFeatureMultinomialEstimator(
			Multinomial.Estimator me) {
	...
	/**
	 * Set the Multinomial Estimator used for priors. The MultinomialEstimator
	 * is internally cloned and the clone is used to maintain the counts that
	 * will be used to generate probability estimates the next time train() or
	 * an initial incrementalTrain() is run. Defaults to a
	 * Multinomial.LaplaceEstimator()
	 ...
	 *            incrementalTrain()
	 */
	public NaiveBayesWithPriorsTrainer setPriorMultinomialEstimator(
			Multinomial.Estimator me) {
		// if (instancePipe != null)
		// throw new
		// IllegalStateException("Can't set after incrementalTrain() is called");
		priorEstimator = me;
		return this;
	}

	/**
	 * Create a NaiveBayes classifier from a set of training data. The trainer
	 * uses counts of each feature in an instance's feature vector to provide an
	 * estimate of p(Labeling| feature). The internal state of the trainer is
	 * thrown away ( by a call to reset() ) when train() returns. Each call to
	 * train() is completely independent of any other.
	 * 
	 * @param trainingList
	 *            The InstanceList to be used to train the classifier. Within
	 *            each instance the data slot is an instance of FeatureVector
	 *            and the target slot is an instance of Labeling
	...
	public NaiveBayes trainIncremental(InstanceList trainingInstancesToAdd) {
		...
		for (Instance instance : trainingInstancesToAdd)
			incorporateOneInstance(instance,
					trainingInstancesToAdd.getInstanceWeight(instance));

		// Estimate multinomials, and return a new naive Bayes classifier.
		// Note that, unlike MaxEnt, NaiveBayes is immutable, so we create a new
		// one each time.
		classifier = new NaiveBayes(instancePipe, pe.estimate(),
				estimateFeatureMultinomials());
		return classifier;
	}
	...
	private void setup(InstanceList instances, Instance instance) {
		...
		if (instances != null) {
			if (instancePipe == null)
				...
				// I don't think so, but it could be confusing to have each
				// returned classifier have a different pipe? -akm 1/08
				throw new IllegalArgumentException(
		...
		if (targetAlphabet.size() > me.length) {
			...
			for (int i = me.length; i < targetAlphabetSize; i++) {
				Multinomial.Estimator mest = (Multinomial.Estimator) featureEstimator
						.clone();
	...
	private void incorporateOneInstance(Instance instance, double instanceWeight) {
		...
		if (labeling == null)
			return; // Handle unlabeled instances by skipping them
		FeatureVector fv = (FeatureVector) instance.getData();
		double oneNorm = fv.oneNorm();
		if (oneNorm <= 0)
			return; // Skip instances that have no features present
		...
		for (int lpos = 0; lpos < labeling.numLocations(); lpos++) {
			...
			if (labelWeight == 0)
				continue;
			// System.out.println ("NaiveBayesTrainer me.increment "+
			// labelWeight * instanceWeight);
	...
	private Multinomial[] estimateFeatureMultinomials() {
		...
		// first add all the appropriate pseudocounts (derived from feature
		// labels)
	...
	/**
	 * Make sure that all keys and values in the background knowledge are
	 * actually in our alphabets!
	 */
	 ...
	 * @param initialClassifier
	 *            Currently unused
	 * @return The NaiveBayes classifier as trained on the trainingList and the
	 *         previous trainingLists passed to incrementalTrain()
	 */
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		if (version != CURRENT_SERIAL_VERSION)
			throw new ClassNotFoundException(
					"Mismatched NaiveBayesTrainer versions: wanted "
							+ CURRENT_SERIAL_VERSION + ", got " + version);
	...
	public static class Factory extends
			ClassifierTrainer.Factory<NaiveBayesTrainer> {
		Multinomial.Estimator featureEstimator = new Multinomial.LaplaceEstimator();
		Multinomial.Estimator priorEstimator = new Multinomial.LaplaceEstimator();
		double docLengthNormalization = -1;

		public NaiveBayesTrainer newClassifierTrainer(
				Classifier initialClassifier) {
			return new NaiveBayesTrainer((NaiveBayes) initialClassifier);
		}

		public NaiveBayesWithPriorsTrainer.Factory setDocLengthNormalization(
				double docLengthNormalization) {
			this.docLengthNormalization = docLengthNormalization;
			return this;
		}

		public NaiveBayesWithPriorsTrainer.Factory setFeatureMultinomialEstimator(
				Multinomial.Estimator featureEstimator) {
			this.featureEstimator = featureEstimator;
			return this;
		}

		public NaiveBayesWithPriorsTrainer.Factory setPriorMultinomialEstimator(
