...
public class SpellCheck extends Pipe {
	...
	/**
	 * Takes the given instance and does a spell check on the tweet text,
	 * replacing misspelled words with the best correction. The spell check is
	 * taken from the Google SpellChecker api
	 */
	@Override
	public Instance pipe(Instance carrier) {
		...
		while (st.hasMoreTokens()) {
			...
			if (token.length() != 1) {
				List<Word> words = (List<Word>) spellchecker.getSuggestions(
						token, threshold);
				Word best = findBest(words);
				token = (best != null && best.getCost() < threshold) ? best
						.getWord() : token;
			}
